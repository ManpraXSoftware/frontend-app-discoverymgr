...
const UpdateDegree = () => {
    ...
    async function getQuickfacts(degree) {
        const response = await getAuthenticatedHttpClient().get(process.env.DISCOVERY_BASE_URL + `/api/v1/quickfacts/?degree=${degree}`)
        if (response.status == 200) {
    ...
    const removedCardImage = (id, fieldName) => {
        ...
        getAuthenticatedHttpClient().patch(process.env.DISCOVERY_BASE_URL + `/api/v1/degree/${id}/`, form_data).then(
            (res) => {
                if (res.status == 200) {

...
const Quickfacts = (props) => {
    ...
    const createQuickFact = async (e) => {
        ...
        getAuthenticatedHttpClient().post(process.env.DISCOVERY_BASE_URL + '/api/v1/quickfacts/', form_data).then(
            (res) => {
                if (res.status == 201) {
    ...
    const deleteQuickFact = (id) => {
        getAuthenticatedHttpClient().delete(process.env.DISCOVERY_BASE_URL + `/api/v1/quickfacts/${id}`).then(
            (res) => {
                if (res.status == 204) {
    ...
    const updateQuickFact = (e) => {
        ...
        getAuthenticatedHttpClient().put(process.env.DISCOVERY_BASE_URL + `/api/v1/quickfacts/${editQuickfactdata.id}/`, quickdata)
            .then((res) => {
                if (res.status == 200){
                    props.get_data(props.degree)
